...
const STORAGE_KEY = 'cvrf-presentation-mode';
...
export function CvrfPanel({ analysisId, readOnly }: CvrfPanelProps) {
  ...
  useEffect(() => {
    ...
    if (stored === 'true') setPresentationMode(true);
  }, []);
  ...
  const activePhase = getPhaseForStep(activeView);

  return (
    <div className={cn('space-y-6', presentationMode && 'text-lg')}>
      {/* Header */}
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className={cn('flex items-center gap-2', presentationMode && 'text-xl')}>
                {analysis.title}
              </CardTitle>
              {presentationMode && activePhase ? (
                <CardDescription className="mt-2 text-base">
                  Fas {activePhase.id}: {activePhase.name} &mdash; Steg {activeView}: {STEP_LABELS[activeView]}
                </CardDescription>
              ) : (
            ...
            >
              ...
              <span className="hidden sm:inline">
                {presentationMode ? 'Avsluta presentation' : 'Presentationsläge'}
              </span>
            </Button>
          </div>
        </CardHeader>
      ...
      {presentationMode ? (
        /* ── Presentation mode: compact horizontal stepper + full-width content ── */
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {PHASES.map((phase, pi) => (
                  <div key={phase.id} className="flex items-center gap-1">
                    {pi > 0 && <div className="h-px w-3 bg-border mx-1" />}
                    <span className="text-xs font-bold text-primary mr-1">{phase.name}</span>
                    {phase.steps.map((step) => (
                      <button
                        key={step}
                        onClick={() => step <= currentStep && setSelectedStep(step)}
                        disabled={step > currentStep}
                        className={cn(
                          'h-8 w-8 rounded-full text-sm font-medium flex items-center justify-center transition-colors',
                          step === activeView && 'bg-primary text-primary-foreground',
                          step < currentStep && step !== activeView && 'bg-primary/20 text-primary hover:bg-primary/30',
                          step === currentStep && step !== activeView && 'bg-muted border-2 border-primary',
                          step > currentStep && 'bg-muted text-muted-foreground/50 cursor-not-allowed'
                        )}
                      >
